...
If the virus scan log file shows no data is being read or scanned, try to scan the disk image using ClamTK, which is installed on the BitCurator machine.

1. Run ClamTK with sudo permissions by opening a terminal window and typing `sudo clamtk` + ENTER
2. In ClamTK, select **Scan a directory**
3. Select the mounted disk image, then select the parent directory to run the virus scan
4. Check the results of the virus scan by double-clicking the **History** icon in ClamTK, select today's date, and click **View** (located on the bottom left of the screen)
5. Copy the results of the virus scan, paste them into a new .txt file
6. Save the .txt file as `ASRefID_scan.txt` inside the folder containing the disk image and log file
